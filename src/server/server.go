...
type ServerOption struct {
	...
	Codec                 tomq_codec.Codec // encodes and decodes the message data, can be nil.
...
func (s *Server) acceptLoop() error {
	...
	for {
		if s.isStopped() {
			logger.Log.Tracef("server accept loop stopped")
		...
		if err != nil {
			...
			if ne, ok := err.(net.Error); ok && !ne.Timeout() {
				logger.Log.Errorf("accept err: %s; retrying in %s", err, tempErrDelay)
